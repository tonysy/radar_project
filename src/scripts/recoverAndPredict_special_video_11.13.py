...
def SaveAndPlotFigure(videoID, faceNumber,recoverVideoPredicted, recoverVideoGrountruth):
    ...
    with h5py.File('./data/specialvideo/{}_grtru_predicted.h5'.format(videoID), 'w') as f:
    ...
    plt.savefig('./data/specialvideo/predictedFigure_{}.png'.format(videoID), dpi=figureDPI)
    print('./data/specialvideo/predictedFigure_{}.png'.format(videoID))
    plt.close()
...
def predictAndPlot(DropoutW, DropoutU, videoIDList,fileListDic, overlap_step, combine_dir):
    ...
    for videoID in videoIDList: #videoID  string 'video1' 'video2'............
        ...
        recoverVideoPredicted = recoverToVideo(True,OVERLAP_STEP, IDnumber, 6, clipNumberTotal, videoPredictedResults)

        recoverVideoGrountruth = recoverToVideoNoAverage(6,groundtruth, OVERLAP_STEP)

        SaveAndPlotFigure(videoID, 6, recoverVideoPredicted, recoverVideoGrountruth)
...
def main():
    ...
    videoIDList = []
    for (dirpath, dirnames,filenames) in os.walk('./data/specialvideo/'):
        videoIDList = dirnames
        break

    predictAndPlot(DropoutW, DropoutU, videoIDList, fileListDic,OVERLAP_STEP,'./data/specialvideo')
