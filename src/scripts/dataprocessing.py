...
def FeatureCombineOverlap(ClipFrameNumber,OverlapStep, combineDataBath, featureDataDir, regressionDir, fileListDic):
    """
            ...
            if zeroFaceNumber >= 0:
                for face in range(1,faceNumber+1):
                    ...
                    for frameIdx in range(clipStart, clipEnd):
                        ...
                        regressionGrTruthAbs = regressionGrTruthAbsGet(ID, frameIdx,face,regressionDir)
                        regressionTemp = np.loadtxt(regressionGrTruthAbs)
                        regressionTemp.shape = (1,1)# get regression groundtruth

                        singleFaceClipFeature = np.vstack([singleFaceClipFeature,featureTemp])
                        singleFaceClipGrTru = np.vstack([singleFaceClipGrTru, regressionTemp])
                ...
                clipMultiFaceSaveFilename = 'combine_video{}_clip{}_featuredim{}.h5'.format(ID,clipIdx,FEATURE_DIM)
...
def readFilenameList(filenameList_FilePath):
    ...
    with open(filenameList_FilePath, 'r') as fi:
        while(True):
            ...
            #fileLabel.append(int(line[1]))
    print('read fileList done, total num :', len(fileName))
    return fileName #, fileLabel
...
def regressionGrTruthAbsGet(videoID, frameNb, faceID, REGRESSION_DIR):

    regressionFileName = 'label_video'+str(videoID)+'_frame'+str(frameNb)+'_face'+str(faceID)+'.txt'
    regressionPathAbs = os.path.join(REGRESSION_DIR, regressionFileName)
    return regressionPathAbs
