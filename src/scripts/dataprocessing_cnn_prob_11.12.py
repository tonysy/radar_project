            ...
            if zeroFaceNumber >= 0:
                for face in range(1,faceNumber+1):
                    ...
                    for frameIdx in range(clipStart, clipEnd):
                        ...
                        grtruKey = regressionDic[grtruKey].tolist()[1]
                ...
                print('Video ID:', ID, ', Clip Index:',clipIdx , 'GroundTruth shape is:', clipGrTruMultiFace.shape)

                if not os.path.exists(os.path.join('./data/cnn_prob/combine_1','video{}'.format(ID))):
                    os.makedirs(os.path.join('./data/cnn_prob/combine_1','video{}'.format(ID)))
                ...
                clipMultiFaceSavePath = os.path.join('./data/cnn_prob/combine_1',clipMultiFaceSaveFilename)
...
def readFilenameList(filenameList_FilePath):
    ...
    with open(filenameList_FilePath, 'r') as fi:
        while(True):
            ...
            CNNProbFileName = line[0].replace('.jpg','_prob.txt')
            CNNProbValue = np.loadtxt(os.path.join('./data/cnn_prob/single',CNNProbFileName))
            #print(CNNProbValue[0],CNNProbValue[1])
            fileGrtru.append(CNNProbValue)
    ...
    # print(type(grtruDic['video1_frame10_face1.jpg'][0]))
