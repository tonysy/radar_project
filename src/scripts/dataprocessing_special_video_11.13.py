...
def FeatureCombineOverlap(ClipFrameNumber,OverlapStep, combineDataBath, featureDataDir, regressionDic, fileListDic):
    ...
    for ID in (videoIDList):
        ...
        zeroFaceNumber = FACE_MAX_2 - faceNumber # zero vetor number
        ID = int(ID)
        ...
        if ID == 13:
            for clipIdx in range(SplitClipMax):
                ...
                if zeroFaceNumber >= 0:
                    ...
                    print(clipFeatureMultiFace[0].shape)

                    # clipFeatureMultiFaceFinal = np.vstack([[clipFeatureMultiFace[0]],[clipFeatureMultiFace[1]],[clipFeatureMultiFace[2]],[clipFeatureMultiFace[3]],[clipFeatureMultiFace[7]],[clipFeatureMultiFace[8]]])
                    # clipGrTruMultiFaceFinal =np.vstack([[clipGrTruMultiFace[0]],[clipGrTruMultiFace[1]],[clipGrTruMultiFace[2]],[clipGrTruMultiFace[3]],[clipGrTruMultiFace[7]],[clipGrTruMultiFace[8]]])
                    # 1 2 3 4 6 8
                    clipFeatureMultiFaceFinal = np.vstack([[clipFeatureMultiFace[0]],[clipFeatureMultiFace[1]],[clipFeatureMultiFace[2]],[clipFeatureMultiFace[3]],[clipFeatureMultiFace[5]],[clipFeatureMultiFace[7]]])
                    clipGrTruMultiFaceFinal =np.vstack([[clipGrTruMultiFace[0]],[clipGrTruMultiFace[1]],[clipGrTruMultiFace[2]],[clipGrTruMultiFace[3]],[clipGrTruMultiFace[5]],[clipGrTruMultiFace[7]]])
                    ...
                    print('Video ID:', ID, ', Clip Index:',clipIdx,' ,Feature shape is:',clipFeatureMultiFaceFinal.shape, 'GroundTruth shape is:', clipGrTruMultiFaceFinal.shape)

                    if not os.path.exists(os.path.join('./data/specialvideo','video{}'.format(ID))):
                        os.makedirs(os.path.join('./data/specialvideo','video{}'.format(ID)))
                    ...
                    clipMultiFaceSavePath = os.path.join('./data/specialvideo',clipMultiFaceSaveFilename)

                    with h5py.File(clipMultiFaceSavePath, 'w') as f:
                        f['data'] = clipFeatureMultiFaceFinal
                        f['grtru'] = clipGrTruMultiFaceFinal
                        f.close()
...
def featureFileAbsGet(videoID, frameNb, faceID, FEATURE_DATA_DIR):
    ...
    featurePathAbs = os.path.join('./../cvpr2017_1024/data/feature_extract/train', featureFileName)
